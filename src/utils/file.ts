...
export function detectIndexHtml(files: any[]): string | false {
  if (!files.length) {
    return false
  }
...
export function getMetadata(files: any[]): any {
...
export function getAssetNameFromFiles(files: any[]): string {
...
async function processItem(item: DataTransferItem, files: any[]) {
...
export async function handleDrop(ev: DragEvent): Promise<any[]> {
  const files: any[] = []
